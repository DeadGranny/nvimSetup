...
local AWS_ACCESS = vim.env.AWS_ACCESS or nil
local AWS_SECRET = vim.env.AWS_SECRET or nil
...
-- Tree Keybinds
-- vim.keymap.set("n", "<leader>t", ":NvimTreeToggle", { desc = "Toggle Tree" })
...
require("lazy").setup({
  ...
  },
  {
    ...
    config = function()
      require("yanky").setup {
        highlight = { timer = 200 }, -- Optional: Highlights yanked text
        ring = { history_length = 50 }, -- Set history length
      }
      -- Yanky keybindings
  ...
  },
  {
    ...
    config = function()
      ...
      -- Set <leader> + t to toggle terminal
